...
# Varsayilan ayarlar: yeni anahtar, kullanicinin ekrandaki degerleri
...
s=s.replace("direk:Number(s.direk||13),hat:Number(s.hat||13),metre:Number(s.hat||13),hatWeight:Number(s.hatWeight||3),\n      direkSymbol:Number(s.direkSymbol||70),trafoSymbol:Number(s.trafoSymbol||200),kofreSymbol:Number(s.kofreSymbol||100),otherSymbol:Number(s.otherSymbol||100),",
            "direk:Number(s.direk||16),hat:Number(s.hat||13),metre:Number(s.hat||13),hatWeight:Number(s.hatWeight||3),\n      direkSymbol:Number(s.direkSymbol||150),trafoSymbol:Number(s.trafoSymbol||260),kofreSymbol:Number(s.kofreSymbol||100),otherSymbol:Number(s.otherSymbol||115),")

# Proje acilisinda sahaNotes govde alani mutlaka hazir olsun
...
old="""  modal.querySelector('#aybNotKaydet').onclick=function(){
    n.tur=tur.value||'Diğer'; n.objeNo=modal.querySelector('#aybNotObje').value.trim(); n.aciklama=modal.querySelector('#aybNotAcik').value.trim(); n.foto=foto; n.updated=new Date().toISOString();
    const arr=aybSahaNotes(); if(!existing) arr.push(n); saveProject(); renderAll(); modal.remove(); setTool(null); toast('Saha notu kaydedildi.');
  };"""
new="""  modal.querySelector('#aybNotKaydet').onclick=function(){
    n.tur=tur.value||'Diğer'; n.objeNo=modal.querySelector('#aybNotObje').value.trim(); n.aciklama=modal.querySelector('#aybNotAcik').value.trim(); n.foto=foto; n.updated=new Date().toISOString();
    n.lat=Number(n.lat); n.lng=Number(n.lng);
    if(!Number.isFinite(n.lat)||!Number.isFinite(n.lng)){toast('Not konumu geçersiz.');return;}
    const arr=aybSahaNotes();
    if(!existing && !arr.some(x=>x&&String(x.id)===String(n.id))) arr.push(n);
    project.sahaNotes=arr; projects[project.id]=project;
    try{saveProject();}catch(e){console.warn('Not saveProject:',e);}
    try{if(typeof aybDepoYaz==='function') aybDepoYaz();}catch(e){console.warn('Not depo:',e);}
    try{renderAll();}catch(e){console.warn('Not render:',e); try{renderSahaNot(n);}catch(_){}}
    modal.remove(); setTool(null);
    setTimeout(function(){
      try{
        var q=document.querySelector('.ayb-saha-note-icon[data-saha-note-id="'+String(n.id).replace(/"/g,'')+'"]');
        if(!q) renderSahaNot(n);
      }catch(e){}
    },80);
    toast('Saha notu kaydedildi.');
  };"""
if old not in s:
    raise SystemExit('Saha notu kaydet blogu bulunamadi')
s=s.replace(old,new)
...
if needle not in s:
    raise SystemExit('Saha notu sil fonksiyonu bulunamadi')
...
pat=re.compile(r"  function results\(\)\{.*?\n  \}\n\n  function highlight",re.S)
m=pat.search(j)
...
res="""  function results(){
    ...
    if(!curTrafo && (curType==='all'||curType==='sahanot') && Array.isArray(p.sahaNotes)){
      p.sahaNotes.forEach(function(n){
        if(!n||!isFinite(+n.lat)||!isFinite(+n.lng)) return;
        var no=String(n.objeNo||n.id||''), tip=String(n.tur||'Saha Notu');
        var hay=low('not '+no+' '+tip+' '+String(n.aciklama||''));
        if(q && hay.indexOf(q)<0) return;
        out.push({o:{lat:+n.lat,lng:+n.lng},no:no||'—',tip:tip,type:'sahanot',id:n.id,n:n});
      });
    }
...
(function(){
  function fix(){
    var row=document.querySelector('.ayb-pro-group.draw .ayb-pro-row'); if(!row)return false;
    var find=function(sel){return row.querySelector(sel)||document.querySelector(sel)};
    var order=[
      '[data-tool="direk"]','[data-tool="trafo"]','[data-tool="yeraltihat"]','[data-tool="hat"]','[data-tool="abonehat"]',
      '[data-tool="kanal"]','[data-tool="kofre"]','[data-tool="bina"]','[data-tool="box"]','[data-tool="sahanot"]',
      '#aybYolOlcBtn','#kfMeasureToolBtn','[data-tool="cizgi"]','[data-tool="ok"]','#aybTopluSilBtn'
    ];
    order.forEach(function(sel){var el=find(sel);if(el&&el.parentNode===row)row.appendChild(el);});
    var h=row.querySelector('[data-tool="hat"] small');if(h)h.textContent='Havai Hat';
    var y=row.querySelector('[data-tool="yeraltihat"] small');if(y)y.textContent='Yeraltı Hat';
    return true;
  }
  var tries=0,iv=setInterval(function(){fix();if(++tries>80)clearInterval(iv);},250);
  document.addEventListener('DOMContentLoaded',fix);
  try{new MutationObserver(function(){fix();}).observe(document.documentElement,{childList:true,subtree:true});}catch(e){}
})();
'''
